...
loadingDataFromFileAndSavingInRda <- function() {
    ...
    close(exdata_data_household_power_consumption)
...
cleaningData <- function(d=dataPlot) {
    ...
    d$DateTime <- paste(c(d$Date, d$Time), sep = " ")
...
dataset <- loadingDataFromFileAndSavingInRda()
dataPlot <- selectingData()
dataPlot <- cleaningData()

#
#hist(dataPlot$Global_active_power)
